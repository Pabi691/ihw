...
const Cart = () => {
  ...
  const selectQuantity = async (itemId, variationId, quantity, price) => {
    ...
    try {
      ...
      await axios.post(
        `${process.env.REACT_APP_API_URL}/api/v1/update_to_cart`,
        {
          product_id: itemId,
  ...
  const removeFromCart = async (cartId) => {
    ...
    try {
      ...
      if (userToken) {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/delete_cart_item/${cartId}`, {
  ...
  useEffect(() => {
    const fetchCoupon = async () => {
      try {
        ...
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/get_suggestion_coupons`, {
          headers: {
  ...
  const proceedHandler = () => {
    if (localStorage.getItem('userToken') && finalAmount > 0) {
      navigate('/checkout', { state: { finalAmount, discount, couponCode } });
    } else {
