...
const OverView = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/v1/get_customer_details`, {
          method: 'GET',
          headers: {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/get_my_orders`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
