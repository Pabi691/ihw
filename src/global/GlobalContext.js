...
export const GlobalProvider = ({ children }) => {
  ...
  useState(() => (localStorage.getItem("userToken") && localStorage.getItem("uservarified") !== null) ? localStorage.getItem("userToken") : webToken);
  ...
  const mergeLocalCartWithServer = useCallback(async (userToken) => {
    ...
    try {
      for (const item of localCart) {
        await axios.post(`${process.env.REACT_APP_API_URL}/api/v1/add_to_cart`, {
          product_id: item.id,
  ...
  const fetchCart = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/get_customer_cart`, {
  ...
  const fetchWishlist = useCallback(async () => {
    ...
    try {
      const { data } = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/get_customer_wishlist`, {
        headers: {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL}/api/v1/get_active_products`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const addToCart = useCallback(async (product, selectedSize) => {
    ...
    if (userToken) {
      ...
      try {
        await axios.post(`${process.env.REACT_APP_API_URL}/api/v1/add_to_cart`, payload, {
          headers: {
            Authorization: `Bearer ${userToken}`,
            "Content-Type": "application/json",
            Accept: "application/json",
  ...
  const addToWishlist = useCallback(async (product) => {
    ...
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL}/api/v1/add_to_wishlist`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  const fetchAllCategories = useCallback(async () => {
    ...
    try {
      ...
      if (cachedPlanters) {
        ...
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/get_slug_data/planters`, {
          headers: { Authorization: `Bearer ${token}` },
        });
      ...
      if (cachedContainer) {
        ...
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/get_slug_data/container`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const data = res?.data?.category_data?.child_categories ?? [];
        setContainer(data);
      ...
      if (cachedAcc) {
        ...
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/get_slug_data/accessories`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const data = res?.data?.category_data?.child_categories ?? [];
