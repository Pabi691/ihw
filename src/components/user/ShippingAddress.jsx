...
const ShippingAddress = () => {
  ...
  useEffect(() => {
    const fetchShipping = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/get_customer_shipping_addresses`, {
          headers: {
  ...
  //   e.preventDefault();
  //   try {
  //     const response = await fetch(`${process.env.REACT_APP_API_URL}/api/v1/add_customer_shipping_addresses`, {
  //       method: "POST",
  //       headers: {
  ...
  const handleSaveAddress = async (e) => {
    ...
    try {
      const url = isEditing
        ? `${process.env.REACT_APP_API_URL}/api/v1/update_customer_shipping_addresses?address_id=${newAddress.id}`
        : `${process.env.REACT_APP_API_URL}/api/v1/add_customer_shipping_addresses`;
  ...
  const handleDeleteAddress = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/v1/delete_customer_shipping_addresses/${addressToCancel}`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
        },
