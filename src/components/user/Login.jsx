...
const Login = () => {
  ...
  const loginUser = async (e) => {
    ...
    setError(null);
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/v1/webuser/login`, {
        method: 'POST',
        headers: {
  ...
  const sendOtp = async (e) => {
    ...
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/v1/forgot-password`, {
        "email": email
      });
  ...
  const resendHandler = async () => {
    ...
    setSuccess(null);
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/resend_email_verification`, {
        headers: {
          "Content-Type": "application/json",
